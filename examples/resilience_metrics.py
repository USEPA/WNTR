...
def topographic_metrics(wn):
    ...
    Nap_density = old_div(float(len(Nap)),uG.number_of_nodes())
    ...
    Nbr_density = old_div(float(len(bridges)),G.number_of_edges())
...
def water_quality_metrics(wn):
    ...
    AGE_at_5hr = old_div(results_AGE.node.loc['quality', 5*3600, :],3600.0)
    wntr.network.draw_graph(wn, node_attribute=AGE_at_5hr, node_size=20, 
                          title='Water age (hrs), time = 5 hours')
    AGE_at_node = old_div(results_AGE.node.loc['quality', :, '208'],3600.0)
    plt.figure()
    ...
    age_last_48h = old_div(age.loc[age.index[-1]-48*3600:age.index[-1]],3600)
    age_last_48h.index = old_div(age_last_48h.index,3600)
