...
print(results.node['pressure'].loc[3600, '123'], results.node['demand'].loc[3600, '123'])

# Access the pressure for all nodes and times
print(results.node['pressure'].loc[ :, :])
...
pressure_at_1hr = results.node['pressure'].loc[ 3600, :]
flowrate_at_1hr = results.link['flowrate'].loc[ 3600, :]
