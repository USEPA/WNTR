...
#        link_segments.to_csv('link_segments_random.csv')
        ...
        expected_node_segments = pd.read_csv(join(test_datadir, 'node_segments_random.csv'), 
                                             index_col=0, squeeze=True)
        expected_link_segments = pd.read_csv(join(test_datadir, 'link_segments_random.csv'), 
                                             index_col=0, squeeze=True)
...
#                                   link_range=[0.5,num_segments+0.5])
    ...
    def test_segmentation_strategic(self):
        # test Net3
        G = self.wn2.get_graph()
        valves = pd.read_csv(join(test_datadir, 'valve_layer_stategic_1.csv'), index_col=0, dtype='object')
...
#        link_segments.to_csv('link_segments_strategic.csv')
        ...
        expected_node_segments = pd.read_csv(join(test_datadir, 'node_segments_strategic.csv'), 
                                             index_col=0, squeeze=True)
        expected_link_segments = pd.read_csv(join(test_datadir, 'link_segments_strategic.csv'), 
                                             index_col=0, squeeze=True)
        expected_node_segments.astype('int32')
        expected_link_segments.astype('int32')
