...
def test_population_impacted(): # tests query and population impacted
    ...
    pop_impacted = wntr.metrics.population_impacted(pop, wsa, np.less_equal, wsa_threshold)
    expected = pd.Series([100,0,300,0,500], index=['J1', 'J2', 'J3', 'J4', 'J5'])
    assert_series_equal(pop_impacted, expected, check_dtype=False)
    ...
    wsa.loc[2,:] = [1,0,1,0,1]
    pop_impacted = wntr.metrics.population_impacted(pop, wsa, np.less, 0.8)
    expected = pd.DataFrame([[100,200,0,0,0],[100,0,300,0,500],[0,200,0,400,0]], 
                            columns=['J1', 'J2', 'J3', 'J4', 'J5'], index=[0,1,2])
    assert_frame_equal(pop_impacted, expected, check_dtype=False)
    

if __name__ == '__main__':
    test_population_impacted()
