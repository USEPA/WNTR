...
from wntr.quality.base import ReactionType, QualityVarType, SpeciesType
from wntr.quality.multispecies import Constant, MultispeciesQualityModel, Parameter, Species, Term
    ...
    def _read_coefficients(self):
        ...
        for lnum, line in self.sections["[COEFFICIENTS]"]:
            ...
            try:
                ...
                typ = QualityVarType.get(vals[0], allow_none=False)
                if typ is QualityVarType.CONSTANT:
                    self.rxn.add_constant(vals[1], float(vals[2]), note=note)
                elif typ is QualityVarType.PARAMETER:
                ...
                fout.write(
                    "{}\n".format(
                        var.note.wrap_msx_string(
                            "{:<12s} {:<8s} {:<32s}".format(var.dynamics_type.name.upper(), str(var.species_name), var.expression)
                        )
                    )
                )
            elif isinstance(var.note, str):
