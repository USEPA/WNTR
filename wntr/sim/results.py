...
class SimulationResults(object):
    ...
    def __sub__(self, other):
        if not isinstance(other, SimulationResults):
            raise ValueError(
                "differencing from a results object requires both be SimulationResults"
            )
    ...
    def convert_units(
        ...
        if qual_param == "CHEMICAL":
            results.link["linkquality"] = from_si(
                flow_units,
                results.link["linkquality"],
                QualParam.Concentration,
                mass_units=mass_units,
            )
        elif qual_param == "AGE":
            results.link["linkquality"] = from_si(
                flow_units, results.link["linkquality"], QualParam.WaterAge
            )
        else:
            pass  # Trace or None, no conversion needed
