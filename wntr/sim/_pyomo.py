...
class ApproxFunctions():
    ...
    def leftLayer(self, x, alpha=1e-5):
        switch_x = self.q1
        return (1 - self.sigmoidFunction(x, switch_x, alpha)) * self.leftFunct(x) + self.sigmoidFunction(x, switch_x,
                                                                                                         alpha) * self.middleFunct(
            x)
...
class PyomoSimulator(WaterNetworkSimulator):
    ...
    def _initialize_from_pyomo_results(self, instance, last_instance_results):
        ...
        for n in instance.tanks_with_leaks:
            ...
            if last_instance_results['head'][n] - node.elevation >= 0.0:
                instance.tank_leak_demand[n].value = node.leak_discharge_coeff * node.leak_area * math.sqrt(
                    2 * self._g) * math.sqrt(last_instance_results['head'][n] - node.elevation)
            else:
        ...
        for n in instance.junctions_with_leaks:
            node = self._wn.get_node(n)
            if last_instance_results['head'][n] - node.elevation >= 0.0:
                instance.junction_leak_demand[n].value = node.leak_discharge_coeff * node.leak_area * math.sqrt(
                    2 * self._g) * math.sqrt(last_instance_results['head'][n] - node.elevation)
            else:
    ...
    def _build_hydraulic_model_at_instant(self,
                                        ...
                                        ELSE=Expr_if(IF=p <= x3, THEN=F2(p),
                                                     ELSE=Expr_if(IF=p <= x4, THEN=smooth_polynomial_2(p),
                                                                  ELSE=Expr_if(IF=p <= x5, THEN=PDD_pyomo(p),
                                                                               ELSE=Expr_if(IF=p <= x6,
                                                                                            THEN=smooth_polynomial_3(p),
                                                                                            ELSE=F4(p)))))))
        ...
        def modified_pump_curve(q, A, B, C):
            ...
            coeff_matrix = np.matrix(
                [[x1 ** 3, x1 ** 2, x1, 1.0], [x2 ** 3, x2 ** 2, x2, 1.0], [3 * x1 ** 2, 2 * x1, 1.0, 0.0],
                 [3 * x2 ** 2, 2 * x2, 1.0, 0.0]])
        ...
        def piecewise_pipe_leak_demand(p, Cd, A):
            ...
            def get_rhs(x, Cd, A):
                return np.matrix([[Cd * A * math.sqrt(2.0 * self._g) * x ** 0.5 - c * x - d],
                                  [0.5 * Cd * A * math.sqrt(2.0 * self._g) * x ** (-0.5) - c]])
        ...
        # Tanks with leaks
        model.tanks_with_leaks = Set(initialize=[n for n, N in wn.nodes(Tank) if
                                                 N.leak_present() == True and current_time_sec >= N.leak_start_time and current_time_sec < N.leak_end_time])

        # Junctions with leaks
        model.junctions_with_leaks = Set(initialize=[n for n, N in wn.nodes(Junction) if
                                                     N.leak_present() == True and current_time_sec >= N.leak_start_time and current_time_sec < N.leak_end_time])
        ...
        def init_tank_leak_rule(model, n):
            ...
            if model.head[n].value - node.elevation >= 0.0:
                return node.leak_discharge_coeff * node.leak_area * math.sqrt(2 * self._g) * math.sqrt(
                    model.head[n] - node.elevation)
            else:
                return 0.0

        model.tank_leak_demand = Var(model.tanks_with_leaks, within=Reals, initialize=init_tank_leak_rule,
                                     bounds=(None, None))
        ...
        model.junction_leak_demand = Var(model.junctions_with_leaks, within=Reals, initialize=init_junction_leak_rule,
                                         bounds=(None, None))
        ...
        for l in model.pipes:
            ...
            if l in links_closed:
                ...
                if modified_hazen_williams:
                    setattr(model, 'pipe_headloss_' + str(l),
                            Constraint(expr=Expr_if(IF=model.flow[l] > 0, THEN=1, ELSE=-1)
                                            * pipe_resistance_coeff * approximator.hazenWDisc(abs(model.flow[l])) ==
                                            model.head[start_node] - model.head[end_node]))
                else:
                    setattr(model, 'pipe_headloss_' + str(l), Constraint(
                        expr=Expr_if(IF=model.flow[l] > 0, THEN=1, ELSE=-1) * pipe_resistance_coeff * (abs(
                            model.flow[l])) ** 1.852 == model.head[start_node] - model.head[end_node]))
        ...
        for l in model.pumps:
            ...
            if l not in links_closed:
                if l in pumps_closed_by_outage:
                    ...
                    pipe_resistance_coeff = self._Hw_k * (200.0 ** (-1.852)) * (
                    1 ** (-4.871)) * 10.0  # Hazen-Williams coefficient
                    if modified_hazen_williams:
                        setattr(model, 'pipe_headloss_' + str(l),
                                Constraint(expr=Expr_if(IF=model.flow[l] > 0, THEN=1, ELSE=-1)
                                                * pipe_resistance_coeff * approximator.hazenWDisc(abs(model.flow[l])) ==
                        ...
                        setattr(model, 'pipe_headloss_' + str(l), Constraint(
                            expr=pipe_resistance_coeff * model.flow[l] * (abs(model.flow[l])) ** 0.852 == model.head[
                                                                                                              start_node] -
                                                                                                          model.head[
                                                                                                              end_node]))
                else:
                    if pump.info_type == 'HEAD':
                        ...
                        if l not in links_closed:
                            setattr(model, 'pump_negative_headloss_' + str(l), Constraint(
                                expr=model.head[end_node] - model.head[start_node] == (
                                modified_pump_curve(model.flow[l], A, B, C))))
                    elif pump.info_type == 'POWER':
                        if l not in links_closed:
                            setattr(model, 'pump_negative_headloss_' + str(l), Constraint(
                                expr=(model.head[start_node] - model.head[end_node]) * model.flow[
                                    l] * self._g * 1000.0 == -pump.power))
                    else:
        ...
        def tank_dynamics_rule(model, n):
            ...
            # of the previous timestep
                ...
                tank = wn.get_node(n)
                return (last_tank_net_inflows[n] * model.timestep * 4.0) / (math.pi * (tank.diameter ** 2)) == \
                       model.head[n] - last_tank_head[n]
        ...
        def pressure_driven_demand_rule(model, j):
            ...
            if model.demand_required[j] == 0.0:
                ...
                return pressure_dependent_demand_nl(model.demand_required[j], model.head[j] - junction.elevation,
                                                    junction.nominal_pressure, junction.minimum_pressure) == \
                       model.demand_actual[j]
        ...
        def tank_leak_rule(model, n):
            node = wn.get_node(n)
            return model.tank_leak_demand[n] == piecewise_pipe_leak_demand(model.head[n] - node.elevation,
                                                                           node.leak_discharge_coeff, node.leak_area)
        ...
        def junction_leak_rule(model, n):
            node = wn.get_node(n)
            return model.junction_leak_demand[n] == piecewise_pipe_leak_demand(model.head[n] - node.elevation,
    ...
    def run_sim(self, solver='ipopt', solver_options={}, modified_hazen_williams=True, fixed_demands=None):
        ...
        while t < self._n_timesteps and step_iter < self._max_step_iter:
            ...
            if step_iter == 0:  # the step_iter == 0 is very important so that the post-solve controls are not overwritten
                if first_timestep:
                    self._apply_controls(None, first_timestep, links_closed_by_controls,
                                         t)  # For the first timestep, we only apply time controls and None is passed in place of last_instance_results
                else:
                    self._apply_controls(last_instance_results, first_timestep, links_closed_by_controls,
                                         t)  # time controls and conditional controls
                if self._pump_outage:
                    self._apply_pump_outage(pumps_closed_by_outage, t)  # pump outage controls
                if not first_timestep:
                    self._close_all_links_for_tanks_below_min_head(last_instance_results,
                                                                   links_closed_by_tank_controls)  # controls for closing links if the tank level gets too low or opening links if the tank level goes back above the minimum head
            ...
            if step_iter == self._max_step_iter:
                ...
                raise RuntimeError('Simulation did not converge at timestep ' + str(t) + ' in ' + str(
                    self._max_step_iter) + ' trials.')
    ...
    def predict_next_tank_head(self, tank_name, instance):
        ...
        new_tank_head = instance['head'][tank_name] + tank_net_inflow * self._hydraulic_step_sec * 4.0 / (
        math.pi * tank.diameter ** 2)
        ...
        for constraint_name in constraint_names:
            ...
            for constraint_key in con.keys():
                ...
                if (con_lower - con_value) >= 1.0e-5 or (con_value - con_upper) >= 1.0e-5:
                    ...
                    print('lower: ', con[constraint_key].lower, '\t body: ', con[constraint_key].body, '\t upper: ',
                          con[constraint_key].upper)
