...
from epanetlib.network.WaterNetwork import *
...
wn.set_graph_node_attribute('elevation', np.greater_equal, 0.0)
wn.set_graph_link_attribute('length', np.greater_equal, 0.0)
...
en.network.draw_graph(G, node_attribute=nzd_nodes,
...
tank_nodes = [name for name, node in wn.nodes.iteritems() if isinstance(node, Tank)]
...
attr = dict(zip(pipes.values(),[1]*len(pipes.values())))
